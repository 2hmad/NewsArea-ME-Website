...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">Entrepreneurs</li>
  </ol>
</nav>
<div class="row">
    <?php
  $sql = "SELECT * FROM articles WHERE (category = 'Entrepreneurs' AND visible > 0) ORDER BY id DESC LIMIT 1";
...
$pic_name = $row['pic_name'];
  ...
  $profile_pic = $row_author['profile_pic'];
    ...
    <a href='article.php?id=$id'>
      <div class='big-card-article' style='width: 775px;height: 422px;'><img src='uploads/$pic_name' class='animated animatedFadeInUp fadeInUp' alt='$title' style='width:100%;height: 100%;object-fit: cover;object-position: center;transition: 1s all ease-in-out;'></div>
        ...
        <p class='card-text' style='color: #6c757d!important'><img src='data:image/jpeg;base64,".base64_encode($profile_pic)."' alt='$author' style='max-width:40px;height:40px;border-radius:50%'> By <small class='text-muted' style='color:black !important;font-weight:bold'>$author</small></p>
    </a>
    </div>
    ";
    }
  ?>
        <div class="col-lg">
            <ul class="latest-articles">
                ...
                <?php
  $sql = "SELECT * FROM articles WHERE (category = 'Entrepreneurs' AND visible > 0) ORDER BY id DESC LIMIT 4";
  $query = mysqli_query($connect, $sql);
  while($row = mysqli_fetch_array($query)){
  $id = $row['id'];
  $title = $row['title'];
  ...
  ?>
    ...
    <div class="row">
    <?php
  $sql = "SELECT * FROM articles WHERE (category = 'Entrepreneurs' AND visible > 0) ORDER BY id DESC";
  $query = mysqli_query($connect, $sql);
  while($row = mysqli_fetch_array($query)){
  $id = $row['id'];
  $user_id = $row['user_id'];
