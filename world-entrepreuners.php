...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">World's Entrepreneurs</li>
  </ol>
</nav>
<div class="row">
    <?php
  $sql = "SELECT * FROM articles WHERE (category = 'World Entrepreneurs' AND visible > 0) ORDER BY id DESC LIMIT 1";
...
$pic_name = $row['pic_name'];
  ...
  ?>
        <div class="col-lg">
            <ul class="latest-articles">
                ...
                <?php
  $sql = "SELECT * FROM articles WHERE (category = 'World Entrepreneurs' AND visible > 0) ORDER BY id DESC LIMIT 4";
  $query = mysqli_query($connect, $sql);
  while($row = mysqli_fetch_array($query)){
  $id = $row['id'];
  $title = $row['title'];
  ...
  ?>
    ...
    <div class="row">
    <?php
  $sql = "SELECT * FROM articles WHERE (category = 'World Entrepreneurs' AND visible > 0) ORDER BY id DESC";
  $query = mysqli_query($connect, $sql);
  while($row = mysqli_fetch_array($query)){
  $id = $row['id'];
  $user_id = $row['user_id'];
